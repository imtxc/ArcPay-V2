...
export default function RequestHub({ address, refetchBalance }: any) {
      ...
      const results = await Promise.allSettled(targetIds.map(async (id) => {
        ...
        if (!nameCache.current[d.requester]) {
           try {
             ...
             nameCache.current[d.requester] = name && name !== "" ? `@${name.toUpperCase()}` : `${d.requester.slice(0,6)}...`;
  ...
  useEffect(() => { syncHub(); }, [address, inIds, syncHub]);
    ...
    <div className="bg-[#0c0e14]/80 border border-white/5 rounded-[44px] p-10 shadow-3xl w-full border-t-white/10 relative flex flex-col backdrop-blur-2xl">
      <div className="flex justify-between items-start mb-12">
        <div className="flex items-center gap-5 text-left">
           ...
           <div>
             <h3 className="text-3xl font-black uppercase italic tracking-tighter text-white leading-none">Payment Hub</h3>
             <p className="text-[10px] font-black text-emerald-500 uppercase tracking-[0.2em] italic mt-2">Protocol Live</p>
           </div>
        </div>
      ...
      <div className="flex items-center justify-between px-2 mb-8">
        <h4 className="text-[12px] font-black text-blue-400 uppercase tracking-[0.4em] italic">Incoming Requests</h4>
        <div className="w-7 h-7 bg-blue-600/10 rounded-full flex items-center justify-center text-[11px] font-black text-blue-500 border border-blue-500/20">{incoming.length}</div>
      </div>
        ...
        {incoming.slice(0, displayLimit).map((req) => (
          <div key={req.id.toString()} className="bg-white/5 border border-white/5 p-8 rounded-[40px] flex items-center justify-between group hover:bg-white/[0.08] transition-all shadow-2xl">
             <div className="flex items-center gap-8 text-left">
                <div className="w-20 h-20 bg-slate-800 rounded-full border-4 border-black/20 overflow-hidden shrink-0 shadow-2xl">
                  <img src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${req.displayUser}`} alt="User Avatar" className="w-full h-full object-cover" />
                </div>
                <div className="space-y-2">
                   <p className="text-xl font-black text-white italic uppercase tracking-tighter leading-none">{req.displayUser}</p>
                   <p className="text-4xl font-black italic text-blue-400 leading-none tracking-tighter">
                     {formatUnits(req.amount, 6)} <span className="text-xl not-italic uppercase tracking-normal">USDC</span>
                   </p>
                   <p className="text-[9px] font-bold text-amber-500/80 uppercase tracking-widest mt-3 flex items-center gap-2 animate-pulse">
                     <Clock size={12}/> Expires in {Math.max(0, Math.floor((86400 - (Math.floor(Date.now() / 1000) - Number(req.timestamp))) / 3600))}h
                   </p>
                </div>
             </div>
      ...
      <div className="mt-12 pt-8 border-t border-white/5 text-center flex items-center justify-center gap-3 opacity-30">
        <Lock size={12}/><p className="text-[9px] font-black uppercase italic text-slate-500 leading-none">Secured Protocol</p>
      </div>
    </div>
  );
}