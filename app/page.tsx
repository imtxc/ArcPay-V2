...
import { 
  ...
  HandCoins, CreditCard, Download, History, User, CheckCircle2, Loader2, Scan, UserPlus, UserSearch, X, Lock
} from 'lucide-react';
...
export default function Dashboard() {
  ...
  const addNotification = (msg: string, type: string) => {
    ...
    const updated = [newNotif, ...notifications].slice(0, 15);
  ...
  const handleEmbeddedPay = async () => {
    ...
    try {
      ...
      if (targetAddr.startsWith('@')) {
         ...
         if (!res || res === '0x0000000000000000000000000000000000000000') {
           showToast("Invalid Identity", "error");
           setIsPaying(false);
           return;
         }
      ...
      showToast("Protocol Aborted", "error");
    } finally {
      setIsPaying(false);
    }
  };
    ...
    return (
      <div className="min-h-screen bg-[#020408] flex flex-col items-center justify-center font-sans">
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center space-y-6">
          ...
          <h1 className="text-2xl font-black italic tracking-widest text-white uppercase text-left">ARCPAY <span className="text-blue-500">V2</span></h1>
          <p className="text-[9px] font-black text-slate-700 uppercase tracking-[0.4em] text-left">ENCRYPTING SECURITY LAYERS...</p>
        </motion.div>
      </div>
    );
  }
  ...
  if (!authenticated) {
    return (
      <div className="min-h-screen bg-[#020408] flex items-center justify-center p-6 text-center text-white font-sans relative overflow-hidden">
        ...
        <div className="max-w-sm w-full relative z-10 text-left">
           <Zap size={60} fill="white" className="mb-10 text-blue-600" />
  ...
  return (
    <div className="min-h-screen bg-[#020408] text-white flex overflow-hidden font-sans selection:bg-blue-600 leading-none">
      ...
      <main className="flex-1 h-screen overflow-y-auto custom-scrollbar p-10 scroll-smooth">
        <div className="max-w-[1600px] mx-auto space-y-10 pb-24 text-left text-white">
          
          <div className="flex justify-between items-center leading-none">
            <div className="space-y-1 text-left">
              <p className="text-slate-500 text-[10px] font-black uppercase tracking-[0.2em] mb-1 italic opacity-50">PERMANENT ON-CHAIN IDENTITY</p>
              <h1 className="text-2xl font-black italic tracking-tighter uppercase flex items-center gap-4 text-white text-left">
                {username ? (
            ...
            <div className="bg-white/5 border border-white/5 p-5 rounded-[32px] flex items-center gap-6 backdrop-blur-3xl shadow-2xl">
               ...
               <div className="text-left leading-none">
                 <p className="text-[8px] font-black uppercase text-slate-500 tracking-widest mb-1">BALANCE</p>
                 <p className="text-xl font-black italic">{usdcBalance} <span className="text-[10px] opacity-20 uppercase font-bold">USDC</span></p>
               </div>
          ...
          <div className="grid grid-cols-12 gap-10 items-start pb-20 text-left">
             <div className="col-span-3 space-y-10">
                <div id="pay-section-anchor" className="bg-[#0c0e14] border border-white/5 rounded-[44px] p-10 space-y-8 shadow-3xl border-t-white/10 text-left">
                   <div className="flex items-center gap-2"><Lock size={12} className="text-slate-700"/><h3 className="text-[12px] font-black uppercase tracking-[0.3em] text-slate-500 italic">PAY BY ID</h3></div>
             ...
             <div className="col-span-9 space-y-10">
                ...
                <div className="bg-[#0c0e14] border border-white/5 rounded-[44px] p-10 shadow-3xl text-center relative overflow-hidden border-t-white/10 flex justify-between items-center mt-10">
                   <div className="text-left space-y-4">
                      <h3 className="text-xl font-black uppercase italic text-white">Global Identity</h3>
      ...
      <SettingsModal isOpen={activeModal === 'settings'} onClose={() => setActiveModal(null)} logout={handleLogout} showToast={showToast} userAddress={address || ""} />
      
      {/* ✅ CORRECT TRIGGER: Dashboard Sidebar 'outgoing' click will open this */}
      <OutgoingModal 
        isOpen={activeModal === 'outgoing'} 
        onClose={() => setActiveModal(null)} 
        address={address || ''} 
      />
      
      <TransactionsModal 
        isOpen={activeModal === "transactions" || activeModal === "ledger"} 
        onClose={() => setActiveModal(null)} 
        userAddress={address || ""} 
